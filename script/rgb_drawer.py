...
class LiftEnv(SapienEnv):
    def __init__(self):
        self.init_qpos = [0, -0.8019634954084936207, 0.0, -2.317993877991494,0.0, 2.941592653589793, 0.6853981633974483, 0, 0]
        ...
        self.drawer = self.get_articulation('drawer')
    ...
    def _build_world(self):
        ...
        drawer = loader.load('../assets/46859/mobility.urdf') 
        drawer.set_name('drawer')
        drawer.set_root_pose(Pose([1.0, 0, 0.0]))
        drawer.set_qpos([0,0])

        # cube
        builder = self._scene.create_actor_builder()
        builder.add_box_collision(half_size=[0.02, 0.02, 0.04])
        builder.add_box_visual(half_size=[0.02, 0.02, 0.04], color=[1, 0, 0])
        cube = builder.build(name='cube')
        cube.set_pose(sapien.Pose([0.7, 0, 0.55]))
        ...
        robot.set_root_pose(Pose([0, 0, 0.1]))
        ...
        # # print(type(ef),ef)
        camera_mount_actor.set_pose(Pose([-1.5, -1.1, 1.8],[ 0.9424921 ,0, 0.2567614, 0.2139678]))
...
#        print("reset:/")
        ...
        self.drawer.set_qpos([0,0])
        self.cube.set_pose(Pose([0.9,0.15,0.55]))
        self.cube_pose= self.cube.get_pose()
        self._scene.step()
        self.curr_step = 0
    ...
    def _get_obs(self):
        ...
        # d_qpos = self.drawer.get_qpos()
        # # cube_pose = self.cube.get_pose()
        # d_pose = self.drawer.get_pose()
    ...
    def _get_reward(self):
        ...
        opening_reward = 1.0* np.sin(self.drawer.get_qpos()[1])
