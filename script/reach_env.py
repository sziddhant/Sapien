...
class ReachEnv(SapienEnv):
    def __init__(self):
        ...
        #Set position of point to reach
        self.goal_pose = Pose([0, 0, 0.02])
        # Threshold for abstracted reach
        self.reach_tresh = 0.1 
        ...
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=[self.dof * 2 + 7*2 +3], dtype=np.float32)
    ...
    def _build_world(self):
        ...
        robot.set_root_pose(Pose([-0.16 - 0.4, 0, 0.02]))
    ...
    def step(self, action):
        ...
        self.episode_no+=1
        done = bool(np.linalg.norm(self.goal_pose.p - self.robot.pose.p) < self.reach_tresh)
        if done:
            reward += 100.0

        if self.episode_no>=self.max_episodes:
            done = True
    ...
    def reset(self):
        ...
        self.goal_pose =Pose(
            [np.random.randn() * 0.05, np.random.randn() * 0.05, 0.8])
    ...
    def _get_obs(self):
        ...
        goal_to_ee = ee_pose.p - goal_pose.p

        return np.hstack([qpos, qvel, ee_pose.p, ee_pose.q, goal_pose.p,goal_pose.q, goal_to_ee])

    def _get_reward(self):
        # reaching reward
        goal_pose = self.goal_pose
        ee_pose = self.end_effector.get_pose()
        distance = np.linalg.norm(ee_pose.p - goal_pose.p)
